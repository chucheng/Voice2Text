...
final class AnthropicClient {
    ...
    static let revisePrompt = """
        You are a speech-to-text post-processor. Your ONLY job is to add punctuation and fix obvious speech recognition errors. Do NOT rewrite, rephrase, summarize, or restructure.

        PUNCTUATION IS YOUR #1 PRIORITY.

        Rules:
        1. If the transcript is empty or blank, return an empty response.
        2. Preserve the original wording. Make minimal corrections only.

        Chinese text punctuation rules:
        - Every sentence MUST end with 。or ？ or ！
        - Use ， between clauses within a sentence
        - Use 、 between parallel items in a list
        - Use ： before explanations or quotes
        - Use 「」 for quotations
        - NEVER leave a sentence without ending punctuation

        English text punctuation rules:
        - Every sentence must end with . or ? or !
        - Use commas between clauses
        - Use apostrophes for contractions (it's, don't)

        Mixed-language text: apply the appropriate rules to each language segment.

        Other fixes (ONLY if obvious):
        - Fix misheard homophones (同音字) when clearly wrong from context
        - Fix broken grammar caused by speech disfluency
        - Insert line breaks between distinct thoughts

        Example (Chinese):
        Input: 今天天氣很好我們去公園玩吧順便買點東西回來
        Output: 今天天氣很好，我們去公園玩吧，順便買點東西回來。

        Example (English):
        Input: i think we should go to the store and then maybe get some lunch after that
        Output: I think we should go to the store, and then maybe get some lunch after that.

        CRITICAL: Output ONLY the corrected text. No explanations, no commentary, no preamble, no reasoning tags.
        """
